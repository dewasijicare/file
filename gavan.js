...
        const mainRow = card ? card.querySelector('.row.mb-3') : null; // Ini adalah <div class="row mb-3">
        const buttonWrapper = card ? card.querySelector('.d-grid.gap-3.mb-3') : null;

        if (!card || !mainRow || !buttonWrapper) return;

        // [LOGIKA ASLI] Pindahkan mainRow dan buttonWrapper ke dalam form
        mainRow.before(form);
        form.append(mainRow);
...
        // --- [ AWAL BLOK PERBAIKAN TATA LETAK ] ---
        // Cari kontainer kolom kedua (col-lg-6) yang berisi input-input
        const inputsContainer = mainRow.querySelector('.col-lg-6:nth-child(2)');
        
        if (inputsContainer) {
            // Ambil semua elemen di dalamnya (input-group, mb-2, dll.)
            const inputsToMove = Array.from(inputsContainer.children);
            
            // Pindahkan setiap elemen tersebut ke luar, tepat sebelum mainRow (sekarang di dalam form)
            inputsToMove.forEach(input => {
                mainRow.before(input);
            });
            
            // Hapus mainRow yang sekarang sudah kosong (atau hanya berisi col-lg-6 yang kosong)
            mainRow.remove();
        }
        // --- [ AKHIR BLOK PERBAIKAN TATA LETAK ] ---

        // [LOGIKA ASLI] Blok ini menata ulang .form-group (jika ada)
...
        // [LOGIKA ASLI] Blok ini menambahkan toggle untuk password
