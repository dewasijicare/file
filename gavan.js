(function() {
    ...
    function initializeBetFormatting() {
        document.querySelectorAll('#betting-page-container input[type="number"][name^="bet"], #betting-page-container input[inputmode="numeric"][name^="bet"]').forEach(originalInput => {
            ...
            const handleInput = () => {
                ...
                if (/^\d*$/.test(rawValue)) {
                    ...
                    if(newLength > originalLength) {
                        ...
                        // Hitung jumlah koma yang ditambahkan sebelum posisi kursor
                        const commasBefore = (formattedValue.substring(0, cursorPosition + diff).match(/,/g) || []).length - (displayInput.value.substring(0, cursorPosition).match(/,/g) || []).length;
                        const newCursorPosition = cursorPosition + diff - commasBefore;
                         try {
                           displayInput.setSelectionRange(newCursorPosition, newCursorPosition);
                         } catch (e) {
                           // Fallback jika setSelectionRange gagal (jarang terjadi)
                           displayInput.setSelectionRange(formattedValue.length, formattedValue.length);
                         }
                    } else if (newLength < originalLength) {
                        const diff = originalLength - newLength;
                         // Hitung jumlah koma yang dihapus sebelum posisi kursor
                        const commasRemovedBefore = (displayInput.value.substring(0, cursorPosition).match(/,/g) || []).length - (formattedValue.substring(0, cursorPosition - diff).match(/,/g) || []).length;
                        const newCursorPosition = Math.max(0, cursorPosition - diff + commasRemovedBefore);
                         try {
                           displayInput.setSelectionRange(newCursorPosition, newCursorPosition);
                         } catch (e) {
                         ...
                         // Jika panjang sama (misal hanya ganti angka tanpa tambah/hapus koma)
                         try {
                    ...
                    // Jika input tidak valid (bukan angka), kembalikan ke nilai sebelumnya yang valid
                    const currentValidValue = formatNumberWithCommas(originalInput.value);
                    const currentCursor = displayInput.selectionStart;
                    displayInput.value = currentValidValue;
                     try {
                        // Coba kembalikan kursor ke posisi sebelum input tidak valid
                        displayInput.setSelectionRange(currentCursor - 1, currentCursor - 1);
                     } catch(e) {
                        //
                     }
                }
            ...
            const handleBlur = () => {
                displayInput.value = formatNumberWithCommas(originalInput.value);
                // Trigger change event pada input asli jika diperlukan oleh script lain
            ...
            // Handle paste
             displayInput.addEventListener('paste', (e) => {
                 ...
                 const sanitizedText = pastedText.replace(/[^0-9]/g, ''); // Hanya ambil angka
    ...
    function fetchRtpWithIframe() { return new Promise((resolve, reject) => { const iframe = document.createElement("iframe"); iframe.src = "/rtp"; iframe.style.display = "none"; iframe.onload = function() { try { const doc = iframe.contentDocument || iframe.contentWindow.document; const gameNodes = doc.querySelectorAll('.row.mb-3.g-1 > div[class*="col-"]'); if (gameNodes.length === 0) return reject(new Error("No games found")); const highRtpGames = Array.from(gameNodes).filter(node => { const progressBar = node.querySelector(".progress-bar-rtp"); return progressBar && parseInt(progressBar.textContent) >= 75 }); if (highRtpGames.length === 0) return reject(new Error("No games with RTP >= 75%")); const gamesData = highRtpGames.map(node => { const linkEl = node.querySelector("a"); const progressBar = node.querySelector(".progress-bar-rtp"); return { link: linkEl.dataset.playurl, image: linkEl.querySelector("img").src, name: linkEl.dataset.gamename, percentage: parseInt(progressBar.textContent), colorClass: Array.from(progressBar.classList).find(cls => cls.startsWith("bg-")) }; }); resolve(gamesData); } catch (error) { reject(error); } finally { document.body.removeChild(iframe); } }; iframe.onerror = function() { reject(new Error("Failed to load RTP iframe")); document.body.removeChild(iframe); }; document.body.appendChild(iframe); }); }
    function displayGacorGames(container, games) { let html = ""; games.forEach(game => { const now = new Date(); let hours = now.getHours(); const minutes = now.getMinutes(); let nextIntervalMinute = Math.ceil((minutes + 1) / 5) * 5; let possibleTimes = []; if (nextIntervalMinute >= 60) { hours = (hours + 1) % 24; nextIntervalMinute = 0; } for (let m = nextIntervalMinute; m < 60; m += 5) { possibleTimes.push(`${String(hours).padStart(2, "0")}:${String(m).padStart(2, "0")}`); } if (possibleTimes.length === 0) { // If it's xx:56-xx:59, next time is next hour :00 hours = (hours + 1) % 24; possibleTimes.push(`${String(hours).padStart(2, "0")}:00`); } const randomTime = possibleTimes[Math.floor(Math.random() * possibleTimes.length)]; html += `<a href="${game.link}" target="_blank" class="gacor-card"><img src="${game.image}" alt="${game.name}"><div class="gacor-info"><strong title="${game.name}">${game.name}</strong><div class="gacor-time"><i class="bi bi-clock-fill"></i> Pola Berlaku Jam: ${randomTime}</div><div class="progress mt-2"><div class="progress-bar-rtp ${game.colorClass}" role="progressbar" style="width: ${game.percentage}%">${game.percentage}%</div></div></div></a>`; }); container.innerHTML = `<h5><i class="bi bi-stars"></i> GAME GACOR SAAT INI</h5><div class="gacor-card-container">${html}</div>`; const gameTitles = container.querySelectorAll(".gacor-info strong"); gameTitles.forEach(title => { const maxLength = 20; if (title.textContent.length > maxLength) { title.textContent = title.textContent.substring(0, maxLength - 3) + "..."; } }); }
    async function injectGacorGame() { if (document.getElementById("gacor-game-sidebar")) return; const sidebar = document.getElementById("sidebar"); if (!sidebar) return; const container = document.createElement("div"); container.id = "gacor-game-sidebar"; container.className = "m-3"; container.innerHTML = '<h5><i class="bi bi-stars"></i> GAME GACOR SAAT INI</h5><div class="gacor-card-placeholder">Mencari sinyal gacor...</div>'; sidebar.appendChild(container); const hasImageBg = !!document.querySelector('#sidebar .d-flex[style*="background-image"]'); const maxGames = hasImageBg ? 1 : 3; try { const allGames = await fetchRtpWithIframe(); const randomGames = allGames.sort(() => 0.5 - Math.random()).slice(0, maxGames); displayGacorGames(container, randomGames); } catch (error) { console.error("Gagal memuat game gacor, menggunakan fallback:", error); const fallbackGame = { link: "/launch-pragmatic/vs20olympgold", image: "https://4n76bph80j.gbgfstie.biz/game_pic/square/200/vs20olympgold.png", name: "Gates of Olympus", percentage: 96, colorClass: "bg-primary" }; displayGacorGames(container, [fallbackGame].slice(0, maxGames)); } }
    ...
    function styleBettingPage() {
            ...
            if (categoryButtons.length > 0) {
                ...
                if (row && !row.classList.contains('category-buttons')) {
                    ...
                    categoryButtons.forEach(btn => {
                        ...
                        if (btn.id === 'btn-4D') { // Assuming 'btn-4D' is the default active
                            btn.classList.add('active');
                        }
                    });
                    // Add event listener to handle active state switching
        ...
        bettingContainer.querySelectorAll('div[id^="panel-"]:not([data-panel-styled="true"])').forEach(panel => {
            panel.dataset.panelStyled = 'true'; // Mark as styled
            panel.classList.remove('bg-dark', 'border', 'rounded-3', 'p-1', 'p-3');
            if (!panel.classList.contains('card')) panel.classList.add('card', 'mb-3'); // Ensure it's a card

            const cardHeader = panel.querySelector('.card-header');
            const cardBody = panel.querySelector('.card-body');

            // If no header/body, create them and move content
            if (!cardHeader && !cardBody) {
                 const infoDiv = panel.querySelector('.mb-3'); // Often contains title info
                 ...
                 newCardHeader.innerHTML = `<i class="bi bi-joystick"></i> ${title}`; // Add icon
                 const newCardBody = document.createElement('div');
                 newCardBody.className = 'card-body';
                 while (panel.firstChild) { // Move all children into the new body
             ...
             // Style the info/description div inside the body
             const infoDivInsideBody = panel.querySelector('.card-body > .mb-3');
             if(infoDivInsideBody) {
                 infoDivInsideBody.classList.remove('bg-dark', 'border', 'rounded-3', 'p-3', 'd-flex', 'align-items-center');
                 infoDivInsideBody.classList.add('info-description'); // Add class for specific styling
                 // Remove potential duplicate title/icon if header already exists
                 const icon = infoDivInsideBody.querySelector('i.bi');
                 if(icon) icon.remove();
                 const descriptionWrapper = infoDivInsideBody.querySelector('.ms-3');
                 if(descriptionWrapper) {
                     descriptionWrapper.classList.remove('ms-3'); // Remove margin if icon is gone
                 }
                 // Hide duplicate strong title if description follows
            ...
            panel.querySelectorAll('.fa-plus').forEach(icon => {
             ...
             panel.querySelectorAll('button[type="reset"].btn-outline-danger').forEach(resetButton => {
                 ...
                 resetButton.classList.add('btn-danger'); // Match theme
             });
             const addRowButton = panel.querySelector('button[onclick="addRow(event)"]');
             if(addRowButton) {
                addRowButton.classList.remove('btn-outline-primary'); // Remove default outline
                // Specific styling for add row button is in CSS
             }
        });
    }
    ...
    // --- FUNGSI YANG DIPERBARUI & DIPERBAIKI ---
    function styleRegisterPage() {
        ...
        // Logika asli untuk menyatukan form
        mainRow.before(form); // Pindahkan form sebelum row
        form.append(mainRow); // Pindahkan row ke dalam form
        form.append(buttonWrapper); // Pindahkan button ke dalam form
        form.querySelectorAll('h3').forEach(h3 => h3.remove()); // Hapus h3 jika ada

        // Logika asli untuk styling .form-group (JIKA ADA - bisa jadi sudah tidak relevan jika HTML sudah diubah)
        form.querySelectorAll('.form-group').forEach(group => {
            ...
            if (icon) {
                ...
                inputGroup.className = 'input-group mb-2'; // Menggunakan mb-2 seperti di HTML Anda
                const iconSpan = document.createElement('span');
                iconSpan.className = 'input-group-text';
                iconSpan.appendChild(icon.cloneNode(true));
                inputGroup.appendChild(iconSpan);
                inputGroup.appendChild(input); // Input asli dipindahkan ke sini
                newElement = inputGroup;
            } else {
                 // Untuk input tanpa ikon (seperti Kode Referral di HTML Anda)
            ...
            input.classList.add('form-control'); // Pastikan class form-control ada
            if (input.type === 'password') {
                const wrapper = document.createElement('div');
                wrapper.className = 'input-wrapper'; // Wrapper untuk toggle icon
                wrapper.appendChild(newElement); // Masukkan input-group ke wrapper
                 group.replaceWith(wrapper); // Ganti .form-group dengan wrapper
            } else {
                group.replaceWith(newElement); // Ganti .form-group dengan input-group/div
            }
        });

        // Logika asli untuk toggle password (Harus dijalankan SETELAH styling .form-group jika ada)
        const passwordInput = form.querySelector('#password');
        const confirmPasswordInput = form.querySelector('#confirmpassword');
        if (passwordInput && confirmPasswordInput) {
            const passwordWrapper = passwordInput.closest('.input-wrapper');
            // Pastikan wrapper konfirmasi juga ada jika diperlukan toggle di sana (meskipun logika saat ini hanya menambah di #password)
            const confirmWrapper = confirmPasswordInput.closest('.input-wrapper');

            // Hapus ikon toggle lama jika ada (mencegah duplikasi)
            passwordWrapper?.querySelector('.password-toggle-icon')?.remove();
            confirmWrapper?.querySelector('.password-toggle-icon')?.remove(); // Hapus juga dari konfirmasi jika terlanjur ada

            if (passwordWrapper && !passwordWrapper.querySelector('.password-toggle-icon')) {
                addPasswordToggle(passwordInput); // Gunakan fungsi addPasswordToggle yang sudah ada
            }
             // Jika ingin toggle juga di konfirmasi, panggil addPasswordToggle(confirmPasswordInput);
             // Namun, logika asli Anda hanya menambah di #password, jadi kita ikuti itu.
             // Pastikan konfirmasi password ADA di dalam .input-wrapper agar toggle bisa ditambahkan jika dipanggil.
             if (confirmPasswordInput && !confirmWrapper) {
                 ...
                 if (parent.classList.contains('input-group')) {
                    ...
                    groupParent.insertBefore(newWrapper, parent);
        ...
        // --- [ AWAL BLOK PERBAIKAN TATA LETAK 2 KOLOM ] ---
        // Cari kolom kiri (yang mungkin kosong)
        const leftColumn = form.querySelector('.row.mb-3 .col-lg-6:first-child');
        if (!leftColumn) return; // Jika struktur row/col tidak ada, batalkan

        // Cari elemen input yang saat ini mungkin langsung di bawah <form>
        // Cari berdasarkan ID, lalu ambil wrapper terluarnya (.input-group atau .input-wrapper)
        const usernameEl = form.querySelector('#username')?.closest('.input-group, .input-wrapper');
        const passwordEl = form.querySelector('#password')?.closest('.input-group, .input-wrapper');
        const confirmPassEl = form.querySelector('#confirmpassword')?.closest('.input-group, .input-wrapper');

        // Pindahkan elemen-elemen tersebut ke kolom kiri JIKA mereka saat ini adalah anak langsung dari form
        if (leftColumn) {
            if (usernameEl && usernameEl.parentElement === form) {
                leftColumn.appendChild(usernameEl);
            }
            if (passwordEl && passwordEl.parentElement === form) {
                leftColumn.appendChild(passwordEl);
            }
            if (confirmPassEl && confirmPassEl.parentElement === form) {
                leftColumn.appendChild(confirmPassEl);
            }
        }
        // --- [ AKHIR BLOK PERBAIKAN TATA LETAK 2 KOLOM ] ---
    }
    ...
    function styleProfilePage() {
        ...
        formGroups.forEach(group => {
            ...
            let valueText = '';
            if (labelText === 'Nama Lengkap' || labelText === 'Credit') { return; } // Skip these
            if (labelText === 'Nama') { labelText = 'Username'; } // Rename label
            ...
            if (!iconClass) return; // Skip if no icon mapping
        ...
        profileForm.prepend(newContentWrapper); // Add the new rows
        formGroups.forEach(group => group.remove()); // Remove old form groups
    }
    ...
    function styleChangePasswordPage() {
        ...
        form.querySelectorAll('.form-group').forEach(group => {
            ...
            const inputGroup = document.createElement('div');
            inputGroup.className = 'input-group mb-3'; // Use mb-3 for spacing

            const iconSpan = document.createElement('span');
            iconSpan.className = 'input-group-text';
            iconSpan.innerHTML = `<i class="bi ${iconClass}"></i>`;

            const clonedInput = input.cloneNode(true); // Clone input to avoid issues
            clonedInput.placeholder = placeholderText; // Set placeholder on the input itself
            clonedInput.classList.add('form-control'); // Ensure class

            inputGroup.appendChild(iconSpan);
            inputGroup.appendChild(clonedInput);

            const wrapper = document.createElement('div');
            wrapper.className = 'input-wrapper'; // Wrapper for toggle icon
            wrapper.appendChild(inputGroup);

            group.replaceWith(wrapper); // Replace original group with the new structure

            // Add password toggle to the new input
            addPasswordToggle(clonedInput);
        });
    }
    ...
    function styleLogoutButton() {
        ...
        if (profileFormLogout && !profileFormLogout.dataset.styled) {
             ...
             profileFormLogout.classList.add('btn-danger'); // Style as danger button
        ...
        if (sidebarLogout && !sidebarLogout.dataset.styled) {
            sidebarLogout.classList.remove('nav-link'); // Remove nav-link style
            sidebarLogout.classList.add('btn', 'btn-danger', 'mx-3', 'my-2'); // Style as button
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const observerCallback = (mutationsList, observer) => {
            // Panggil semua fungsi styling/initialization di sini
            // Ini memastikan elemen baru yang muncul (misalnya via AJAX) juga di-style
            ...
            initializeBetFormatting(); // Format input taruhan

            // Tambahkan toggle ke semua input password baru yang muncul
            document.querySelectorAll('input[type="password"]:not([data-toggle-added="true"])').forEach(input => {
                // Pastikan input memiliki parent .input-wrapper sebelum menambah toggle
                let wrapper = input.closest('.input-wrapper');
                if (!wrapper) {
                    // Jika belum ada wrapper, buat satu
                    const parent = input.parentElement;
                     // Cek apakah parent adalah input-group, jika iya, bungkus input-group nya
                     if (parent.classList.contains('input-group')) {
                         const groupParent = parent.parentElement; // Parent dari input-group
                         wrapper = document.createElement('div');
                         wrapper.className = 'input-wrapper';
                         groupParent.insertBefore(wrapper, parent); // Masukkan wrapper sebelum input-group
                         wrapper.appendChild(parent); // Pindahkan input-group ke dalam wrapper
                     } else {
                         // Jika parent bukan input-group, bungkus inputnya saja
                         wrapper = document.createElement('div');
                         wrapper.className = 'input-wrapper';
                 ...
                 // Sekarang tambahkan toggle jika wrapper sudah ada
                 if(wrapper) {
        ...
        // Buat dan konfigurasi observer
        observer = new MutationObserver(observerCallback);
        observer.observe(document.body, { childList: true, subtree: true });

        // Panggil callback sekali di awal untuk men-style halaman saat pertama load
        observerCallback();

        // Event listener untuk perubahan bank (deposit) - tetap di luar observer
        document.body.addEventListener('change', (event) => {
            if (event.target.id === 'agentmemberbankid') {
                const receiverBankSpan = document.getElementById('receiver-bank');
                if (receiverBankSpan) delete receiverBankSpan.dataset.iconApplied; // Reset ikon bank
                 // Panggil update UI lagi jika diperlukan segera setelah bank berubah
                 const depositForm = document.querySelector('#deposit-form');
                 if(depositForm) updateDepositFormUI(depositForm);
            }
        });

        // Observer untuk toggle sidebar button - tetap di luar observer utama
